...
<div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Meru Climbing</h1>
            <p class="text-gray-600">Manage Mount Meru climbing expeditions and routes</p>
        </div>
        <div class="flex flex-wrap gap-3 mt-4 lg:mt-0">
            <a href="{{ route('admin.tours.create') }}?tour_type=mountain&location=meru" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-sm font-medium">
                <i class="fas fa-plus mr-2"></i>Add Meru Route
            </a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            ...
            <div class="space-y-1">
                <h3 class="text-2xl font-bold text-gray-900">3.5</h3>
    ...
    <div class="bg-gradient-to-r from-purple-600 to-blue-600 rounded-xl p-6 mb-8 text-white">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <h3 class="text-lg font-semibold mb-2">Mount Meru</h3>
                <p class="text-purple-100 text-sm">Second highest peak in Tanzania at 4,566 meters</p>
            </div>
            <div>
                <h3 class="text-lg font-semibold mb-2">Perfect Acclimatization</h3>
                <p class="text-purple-100 text-sm">Ideal for acclimatization before Kilimanjaro climbs</p>
            </div>
            <div>
                <h3 class="text-lg font-semibold mb-2">Wildlife Paradise</h3>
                <p class="text-purple-100 text-sm">Located in Arusha National Park with diverse wildlife</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-shadow">
            <!-- Route Image/Preview -->
            <div class="h-48 bg-gradient-to-br from-purple-700 to-blue-700 rounded-t-xl relative">
                @if(!empty($route->images))
                    <img src="{{ asset($route->images[0] ?? '/images/default-meru.jpg') }}" alt="{{ $route->name }}" 
                         class="w-full h-full object-cover rounded-t-xl">
                @else
                    <div class="absolute inset-0 flex items-center justify-center text-white">
                        <div class="text-center">
                            <i class="fas fa-mountain text-4xl mb-2"></i>
                            <p class="text-sm">Meru Route</p>
                        </div>
                    </div>
                @endif
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        {{ $route->max_altitude ?? '4,566m' }}
                    </div>
                    <div class="flex items-center text-sm text-gray-600">
                        <i class="fas fa-users mr-2 text-gray-400"></i>
                        {{ $route->max_group_size ?? 8 }} max
                    </div>
                    <div class="flex items-center text-sm text-gray-600">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('admin.tours.show', $route) }}" class="flex-1 px-3 py-2 bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-colors text-sm font-medium text-center">
                        View
                    </a>
                    ...
                    <button onclick="viewRouteDetails({{ $route->id }})" class="px-3 py-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors text-sm font-medium">
    ...
    <div class="bg-white rounded-xl shadow-sm p-12 text-center border border-gray-100">
        ...
        <h3 class="text-xl font-semibold text-gray-900 mb-2">No Meru Routes Found</h3>
        <p class="text-gray-600 mb-6">Get started by adding your first Mount Meru climbing route</p>
        <a href="{{ route('admin.tours.create') }}?tour_type=mountain&location=meru" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
            <i class="fas fa-plus mr-2"></i>Add Meru Route
        </a>
    </div>
    @endif
...
<div id="route-details-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-semibold text-gray-900">Meru Route Details</h3>
        ...
        .then(data => {
            ...
            content.innerHTML = `
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">${data.max_altitude || '4,566m'}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Max Group Size:</span>
                                <span class="font-medium">${data.max_group_size || 8}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600">Minimum Age:</span>
                                <span class="font-medium">${data.min_age || 12}+ years</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-purple-600">$${number_format(data.base_price, 0)}</span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium">Year-round</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ${data.highlights.map(highlight => 
                            `<span class="px-3 py-1 bg-purple-100 text-purple-700 text-sm rounded-full">${highlight}</span>`
                        ).join('')}
                    </div>
                </div>
                ` : ''}
                ...
                <div class="mt-6">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ${data.what_to_bring.map(item => 
                            `<div class="flex items-center text-sm text-gray-600">
                                <i class="fas fa-check text-purple-500 mr-2"></i>${item}
                            </div>`
                        ).join('')}
                    </div>
                </div>
                ...
                <div class="mt-6 p-4 bg-purple-50 rounded-lg">
                    <h4 class="font-semibold text-purple-900 mb-2">Why Choose Meru?</h4>
                    <ul class="text-sm text-purple-800 space-y-1">
                        <li>• Perfect acclimatization for Kilimanjaro</li>
                        <li>• Less crowded than Kilimanjaro</li>
                        <li>• Stunning views of Kilimanjaro</li>
                        <li>• Diverse wildlife in Arusha National Park</li>
                        <li>• Challenging but achievable climb</li>
                    </ul>
                </div>
            `;
...
function exportRoutes() {
    // Export routes data
    console.log('Exporting Meru routes');
}
