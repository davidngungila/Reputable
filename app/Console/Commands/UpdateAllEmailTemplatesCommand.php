...
class UpdateAllEmailTemplatesCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'update:all-email-templates';
     ...
     * @var string
     */
    protected $description = 'Apply FeedTan CMG live structure to all remaining email templates';
    ...
    {
        $this->info("🎨 Applying FeedTan CMG live structure to all email templates...");
        
        $templates = [
            'system/booking-created.blade.php' => 'Booking Created',
            'system/booking-paid.blade.php' => 'Booking Paid',
            'system/itinerary.blade.php' => 'Itinerary',
            'system/otp.blade.php' => 'OTP',
            'system/test.blade.php' => 'Test',
        ];
        ...
        foreach ($templates as $template => $name) {
            ...
            if (File::exists($templatePath)) {
                // Create backup
                $backupPath = resource_path("views/emails/{$template}.backup");
                File::copy($templatePath, $backupPath);
                
                // Update template with FeedTan CMG structure
                $this->updateTemplateWithFeedTanCmgt($templatePath, $name);
                
                $this->info("✅ {$name} updated");
            } else {
                $this->warn("⚠️ Template not found: {$template}");
            }
        }
        
        $this->info("\n🎉 All email templates updated with FeedTan CMG live structure!");
        $this->info("📧 Features applied:");
        $this->info("   - Green theme (#006400)");
        $this->info("   - Poppins typography");
        $this->info("   - Professional layout");
        $this->info("   - Swahili localization");
        $this->info("   - Responsive design");
        
        return 0;
    }
    
    private function updateTemplateWithFeedTanCmgt($templatePath, $templateName)
    {
        $content = File::get($templatePath);
        
        // Basic FeedTan CMG structure for different template types
        $newContent = $this->generateTemplateContent($templateName);
        
        File::put($templatePath, $newContent);
    }
    
    private function generateTemplateContent($templateName)
    {
        $baseStructure = '<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ $title ?? "' . $templateName . ' - Reputable Tours" }}</title>
...
<body>
    <div class="email-container">
        ...
        <div class="content">
            <p class="greeting">Habari {{ $name ?? "Mteja" }},</p>
            <p style="font-size: 14px; color: #4a5568;">{{ $message ?? "Tunatumia ujumbe huu kutoka kwa Reputable Tours kwa ajili ya " . $templateName . "." }}</p>

            <div class="card">
                <div class="card-header">
                    <span class="icon">📧</span>
                    <h4>{{ $title ?? $templateName }}</h4>
                </div>
                <p style="font-size: 14px; color: #4a5568;">Hii ni barua pepe ya mfumo kutoka Reputable Tours inayotumia muundo wa FeedTan CMG.</p>
            ...
            <div class="special-section">
                ...
                <p>Barua pepe hii imetumwa kiotomatiki kutoka kwa mfumo wa Reputable Tours. Ikiwa hii ni kosa, tafadhali puauza barua pepe hii.</p>
            </div>
